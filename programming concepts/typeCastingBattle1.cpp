...
int main(){
    ...
    // converts the integer s to a float before printing. This means 12 is converted to 12.0 internally, but how it's printed depends on cout.
    //Default cout Behavior:
    // By default, cout may display 12.0 as 12 because it's a whole number. C++ might not show the .0 unless you specifically instruct it to do so.(another file for this)
    ...
    // he reason 1234.56789 is printed as 1234.57 is due to the rounding rule, where digits are rounded up if the following digit is 5 or greater. In this case, the digit after 6 is 7, so 1234.56789 rounds up to 1234.57.
}